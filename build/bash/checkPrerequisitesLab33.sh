...
do
  if [[ $(echo ${item} | jq -c -r '.vtype') == "CLOUD_NSXT" && $(echo ${item} | jq -c -r '.nsxt_configuration.data_network_config.tz_type') == "OVERLAY" ]]; then
    ...
    if [[ $(echo "${response_body}" | jq -c -r .results[0].runtime.oper_status.state) == "OPER_UP" && $(echo "${response_body}" | jq -c -r .results[0].config.cloud_ref) == "${nsx_cloud_url}" ]]; then
      vs_uuid_a=$(echo ${response_body} | jq -c -r '.results[0].config.uuid')
    fi
    #
  fi
...
do
  if [[ $(echo ${item} | jq -c -r '.vtype') == "CLOUD_NSXT" && $(echo ${item} | jq -c -r '.nsxt_configuration.data_network_config.tz_type') == "VLAN" ]]; then
    ...
    if [[ $(echo "${response_body}" | jq -c -r .results[0].runtime.oper_status.state) == "OPER_UP" && $(echo "${response_body}" | jq -c -r .results[0].config.cloud_ref) == "${nsx_cloud_url}" ]]; then
      vs_uuid_b=$(echo ${response_body} | jq -c -r '.results[0].config.uuid')
    fi
    #
  fi
done< <(echo "${response_body}" | jq -c -r .results[])
...
csrftoken=$(cat ${avi_cookie_file} | grep csrftoken | awk '{print $7}')
#
if [[ ! -z "$vs_uuid_a" && ! -z "$vs_uuid_b" ]]; then
  avi_api 2 2 "GET" "${avi_cookie_file}" "${csrftoken}" "admin" "${avi_version}" "" "${fqdn}" "api/gslb"
  if [[ $(echo "${response_body}" | jq -c -r '.results[0].dns_configs[0].domain_name') == "gslb.vclass.local" ]]; then
    gslb_domain=ok
  fi
  if [[ $(echo "${response_body}" | jq -c -r '.results[0].sites[0].dns_vses[0].dns_vs_uuid') == ${vs_uuid_a} ]]; then
    gslb_dns_a=ok
  fi
  if [[ $(echo "${response_body}" | jq -c -r '.results[0].sites[1].dns_vses[0].dns_vs_uuid') == ${vs_uuid_b} ]]; then
    gslb_dns_b=ok
  fi
else
  exit 1
...
#
#
if [[ ${gslb_domain} == "ok" && ${gslb_dns_a} == "ok" && ${gslb_dns_b} == "ok" ]]; then
  sleep 1
  exit 0
else
  exit 1
