...
do
  if [[ $(echo ${item} | jq -c -r '.vtype') == "CLOUD_NSXT" ]]; then
      ...
      "dns_info": [
        {
          ...
          "fqdn": "nsx-overlay-vs-tenant01.sa.vclass.local",
          "ttl": 30,
          "type": "DNS_RECORD_A"
        }
      ],
    ...
    avi_api 2 2 "POST" "${avi_cookie_file}" "${csrftoken}" "tenant01" "${avi_version}" "${json_data}" "${fqdn}" "api/vsvip"
    ...
    avi_api 2 2 "POST" "${avi_cookie_file}" "${csrftoken}" "tenant01" "${avi_version}" "${json_data}" "${fqdn}" "api/pool"
    ...
    avi_api 2 2 "POST" "${avi_cookie_file}" "${csrftoken}" "tenant01" "${avi_version}" "${json_data}" "${fqdn}" "api/virtualservice"
    #
    # Recreating nsx-overlay-vs-vip in tenant02
    #
    json_data='
    {
      "cloud_ref": "https://sa-avicon-01.vclass.local/api/cloud/'${nsx_cloud_uuid}'",
      "dns_info": [
        {
          "algorithm": "DNS_RECORD_RESPONSE_CONSISTENT_HASH",
          "fqdn": "nsx-overlay-vs-tenant02.sa.vclass.local",
          "ttl": 30,
          "type": "DNS_RECORD_A"
        }
      ],
    ...
    avi_api 2 2 "POST" "${avi_cookie_file}" "${csrftoken}" "tenant02" "${avi_version}" "${json_data}" "${fqdn}" "api/vsvip"
    vsvip_url=$(echo ${response_body} | jq -c -r '.url')
    #
    # Recreating pool for nsx-overlay-vs
    #
    ...
    avi_api 2 2 "POST" "${avi_cookie_file}" "${csrftoken}" "tenant02" "${avi_version}" "${json_data}" "${fqdn}" "api/pool"
    pool_url=$(echo ${response_body} | jq -c -r '.url')
    #
    # Recreating the VS
    #
    ...
    avi_api 2 2 "POST" "${avi_cookie_file}" "${csrftoken}" "tenant02" "${avi_version}" "${json_data}" "${fqdn}" "api/virtualservice"
  fi
done